...
export default function FAQPage() {
  ...
  return (
    <main className="flex flex-col items-center justify-center min-h-screen bg-zinc-50 p-1 text-zinc-900">
      <div className="flex items-start justify-center flex-col h-full mx-auto max-w-4xl rounded-lg border border-gray-200 bg-white p-8">
          ...
          {categories.map((category) => (
            <div key={category.category_id} className="w-full">
              ...
              >
                ...
                <span className="ml-2 text-gray-500 py-1 px-6">
                ...
                <div className="space-y-4 pl-4 border-l-2 border-gray-200">
                  {category.faqs.map((faq) => (
                    <div key={faq.id} className="border-b border-gray-200 pb-4">
                      <h3 className="text-lg font-medium font-semibold">{faq.question}</h3>
                      <p className="mt-2 text-gray-700 text-wrap">{faq.response}</p>
                    </div>
                  ))}
                </div>
              )}
