...
export default function ChatBox() {
  ...
  const [currentAI, setCurrentAI] = useState<AgentCardProps | null>({
    ...
    imageUrl:
      "/image/lucid-origin_A_futuristic_AI_presence_non-physical_form._Abstract_energy_shape_subtle_glowing-3.jpg",
  ...
  const [keywordChoices, setKeywordChoices] = useState<QuestionChoice[] | null>(
    null,
  );
  const [showKeywordQuestion, setShowKeywordQuestion] = useState(false);
  const [exhibitorChoices, setExhibitorChoices] = useState<
    QuestionChoice[] | null
  >(null);
  const [showExhibitorQuestion, setShowExhibitorQuestion] = useState(false);
  const [exhibitorsList, setExhibitorsList] = useState<Stand[]>([]);
  const [selectedExhibitorForModal, setSelectedExhibitorForModal] = useState<
    Stand | ExhibitorWithStands | null
  >(null);
  const [showExhibitorModal, setShowExhibitorModal] = useState(false);
  const [sponsorChoices, setSponsorChoices] = useState<QuestionChoice[] | null>(
    null,
  );
  ...
  const [conferenceChoices, setConferenceChoices] = useState<
    QuestionChoice[] | null
  >(null);
  const [showConferenceQuestion, setShowConferenceQuestion] = useState(false);
  const [conferenceQuestionLabel, setConferenceQuestionLabel] = useState(
    "Choisissez une conférence :",
  );
  const [selectedAtelierOrganizer, setSelectedAtelierOrganizer] = useState<
    string | null
  >(null);
  ...
  >(null);
  const [
    selectedSpeedRecruitingOrganizer,
    setSelectedSpeedRecruitingOrganizer,
  ...
  >([]);
  const [foodCourtChoices, setFoodCourtChoices] = useState<
    QuestionChoice[] | null
  >(null);
...
// }, [status, router, session]);
  ...
  const fetchSponsors = async () => {
    ...
    try {
      ...
      const sponsorNames = uniqueSponsorStands.map(
        (sponsor) => sponsor.exhibitorName || "Sponsor inconnu",
      );
      ...
      if (events.length === 0) {
        ...
        if (eventType === "Atelier") {
          const organizerNames = Array.from(
            new Set(
              normalizedEvents
                .map((event) => event.organizerOrBrand)
                .filter(Boolean),
            ),
          );
  ...
  const fetchExhibitorsByKeyword = async (keywordId: string) => {
    ...
    try {
      const res = await fetch(
        `/api/exhibitors?keywordId=${encodeURIComponent(keywordId)}`,
      );
      const data = await res.json();
  ...
  const fetchExhibitorDetails = async (exhibitorId: string) => {
    ...
    try {
      const res = await fetch(
        `/api/exhibitors?exhibitorId=${encodeURIComponent(exhibitorId)}`,
      );
      const data = await res.json();
      const stands: Array<{
        exhibitor_id: number;
      ...
      console.error(
        "Erreur lors de la récupération des détails de l'exposant :",
        error,
      );
      setMessages((prev) => [
        ...prev,
      ...
      const interval = setInterval(() => {
        ...
        if (index >= fullText.length) {
          ...
          if (data.stands && data.stands.length > 0) {
            setTimeout(() => {
              const convertedStands: Stand[] = data.stands.map(
                (stand: any) => ({
  ...
  const canGoBack =
    showExhibitorQuestion ||
    showKeywordQuestion ||
    showConferenceQuestion ||
    showSponsorQuestion ||
    showInfoQuestion ||
    showFoodCourtQuestion;
  ...
  const handleChooseOption = async (choice: QuestionChoice) => {
    ...
    if (
      currentQuestion.questionId === 100 &&
      selectedMainTopic === "exhibitor"
    ) {
    ...
    if (
      currentQuestion.questionId === 101 &&
      selectedMainTopic === "exhibitor"
    ) {
    ...
    if (currentQuestion.questionId === 102) {
      if (selectedMainTopic === "Atelier" && !selectedAtelierOrganizer) {
        ...
        setConferenceChoices(
          filteredEvents.map((event, index) => ({
            ...
            description: event.description, 
            horaire: `Horaire: ${formatFrenchTime(event.startTime)} - ${formatFrenchTime(event.endTime)} `,
            value: event.eventId,
          })),
        );
      ...
      if (selectedMainTopic === "Speed recruiting") {
        ...
        if (selectedSpeedRecruitingOrganizer !== organization) {
          ...
          const speedRecruitingEvent = conferencesList.find(
            (event) =>
              event.organizerOrBrand === organization ||
              event.organisateur === organization,
          );
      ...
      if (
        (selectedMainTopic === "Conférence" ||
          selectedMainTopic === "Table ronde") &&
        ...
        setConferenceQuestionLabel(
          `Choisissez une ${selectedMainTopic.toLowerCase()} :`,
        );
        setShowConferenceQuestion(true);
        setMessages((prev) => [
          ...prev,
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-base font-semibold text-gray-900">
                  {"stands" in selectedExhibitorForModal
                    ? selectedExhibitorForModal.exhibitorName
                    : selectedExhibitorForModal.exhibitorName}
                </p>
              </div>
